...
tree.y2.zero <- ctree(is.zero ~ n_claims+Sex+AgeAtFirstClaim+sum_pay_delay+CharlsonSum, data=y2df)
...
#trainy2 <- y2df[y2df$tree.y1.zero < .8,1:92]
# Want to focus the model on a tight range of important values
# zeros and big vals create too much bias 
trainy2 <- y2df[(y2df$days_in_hospital > 0) & (y2df$days_in_hospital < 4.0),1:92]
ctreey2 <- ctree(days_in_hospital ~ ., data=trainy2)
print("ctree")
model_perfect_zeros(y1df,y2df,ctree,ctreey2)

tree <- tree(days_in_hospital ~ ., data=train)
treey2 <- tree(days_in_hospital ~ ., data=trainy2)
print("tree")
model_perfect_zeros(y1df,y2df,tree,treey2)

lm <- lm(days_in_hospital ~ ., data=train)
lmy2 <- lm(days_in_hospital ~ ., data=trainy2)
print("LM")
model_perfect_zeros(y1df,y2df,lm,lmy2)


stop()

train <- y1df[y1df$tree.y1.zero < .8,1:92]
t1 <- tree(days_in_hospital ~ ., data=train)
trainy2 <- y2df[y2df$tree.y1.zero < .8,1:92]
t2 <- tree(days_in_hospital ~ ., data=trainy2)
model_perfect_zeros(y1df,y2df,t1,t2)
model_and_report(y1df,y2df,t1,t2)
...
#submission_file <- "../submissions/target_20130331_y2.csv"
#y3df$SupLOS <- 0
